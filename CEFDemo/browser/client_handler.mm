...
namespace client {
  ...
  namespace {
    ...
    std::string GetCertStatusString(cef_cert_status_t status) {
      ...
      CEFM_FLAG(CERT_STATUS_COMMON_NAME_INVALID);
      CEFM_FLAG(CERT_STATUS_DATE_INVALID);
      CEFM_FLAG(CERT_STATUS_AUTHORITY_INVALID);
      CEFM_FLAG(CERT_STATUS_NO_REVOCATION_MECHANISM);
      CEFM_FLAG(CERT_STATUS_UNABLE_TO_CHECK_REVOCATION);
      CEFM_FLAG(CERT_STATUS_REVOKED);
      CEFM_FLAG(CERT_STATUS_INVALID);
      CEFM_FLAG(CERT_STATUS_WEAK_SIGNATURE_ALGORITHM);
      CEFM_FLAG(CERT_STATUS_NON_UNIQUE_NAME);
      CEFM_FLAG(CERT_STATUS_WEAK_KEY);
      CEFM_FLAG(CERT_STATUS_PINNED_KEY_MISSING);
      CEFM_FLAG(CERT_STATUS_NAME_CONSTRAINT_VIOLATION);
      CEFM_FLAG(CERT_STATUS_VALIDITY_TOO_LONG);
      CEFM_FLAG(CERT_STATUS_IS_EV);
      CEFM_FLAG(CERT_STATUS_REV_CHECKING_ENABLED);
      CEFM_FLAG(CERT_STATUS_SHA1_SIGNATURE_PRESENT);
      CEFM_FLAG(CERT_STATUS_CT_COMPLIANCE_FAILED);
    ...
    std::string GetSSLVersionString(cef_ssl_version_t version) {
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_UNKNOWN);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_SSL2);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_SSL3);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_TLS1);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_TLS1_1);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_TLS1_2);
      CEFM_VALUE(version, SSL_CONNECTION_VERSION_QUIC);
      return std::string();
    }
    ...
    std::string GetContentStatusString(cef_ssl_content_status_t status) {
      ...
      CEFM_VALUE(status, SSL_CONTENT_NORMAL_CONTENT);
      CEFM_FLAG(SSL_CONTENT_DISPLAYED_INSECURE_CONTENT);
      CEFM_FLAG(SSL_CONTENT_RAN_INSECURE_CONTENT);
  ...
  delegate_(delegate),
  browser_count_(0),
  console_log_file_(BrowserManager::Get()->GetConsoleLogPath()),
  ...
  void ClientHandler::OnFaviconURLChange(CefRefPtr<CefBrowser> browser, const std::vector<CefString>& icon_urls) {
    CEF_REQUIRE_UI_THREAD();
    if (!icon_urls.empty()) {
      browser->GetHost()->DownloadImage(icon_urls[0], true, 16, false, new ClientDownloadImageCallback(this));
    }
  }
  ...
  void ClientHandler::OnDownloadUpdated(CefRefPtr<CefBrowser> browser,
    ...
    if (download_item->IsComplete()) {
      utils::Alert(browser, "File \"" + download_item->GetFullPath().ToString() + "\" downloaded successfully.");
    }
  }
  ...
  bool ClientHandler::OnBeforePopup(CefRefPtr<CefBrowser> browser,
    ...
    BrowserManager::Get()->CreateRootWindowAsPopup(WindowType_Other,
                                                                        popupFeatures,
                                                                        windowInfo,
                                                                        client,
                                                                        settings);
  ...
  bool ClientHandler::OnOpenURLFromTab(CefRefPtr<CefBrowser> browser,
    ...
    if (target_disposition == WOD_NEW_BACKGROUND_TAB ||
      ...
      // browser window.
      BrowserManager::Get()->CreateRootWindow(WindowType_Web, false, target_url);
      return true;
    }
  ...
  cef_return_value_t ClientHandler::OnBeforeResourceLoad(CefRefPtr<CefBrowser> browser,
                                                         CefRefPtr<CefFrame> frame,
                                                         CefRefPtr<CefRequest> request,
  ...
  bool ClientHandler::OnSelectClientCertificate(CefRefPtr<CefBrowser> browser,
                                                bool isProxy,
    ...
    const std::string& cert_name = command_line->GetSwitchValue(switches::kSslClientCertificate);
    ...
    std::vector<CefRefPtr<CefX509Certificate>>::const_iterator it = certificates.begin();
  ...
  void ClientHandler::ShowDevTools(CefRefPtr<CefBrowser> browser, const CefPoint& inspect_element_at) {
    ...
    if (!has_devtools) {
      ...
      BrowserManager::Get()->CreateRootWindowAsPopup(WindowType_DevTools,
                                                                          CefPopupFeatures(),
                                                                          windowInfo,
                                                                          client,
                                                                          settings);
  ...
  bool ClientHandler::HasSSLInformation(CefRefPtr<CefBrowser> browser) {
    ...
    return (nav && nav->GetSSLStatus() && nav->GetSSLStatus()->IsSecureConnection());
  }
  ...
  void ClientHandler::ShowSSLInformation(CefRefPtr<CefBrowser> browser) {
    ...
    BrowserManager::Get()->CreateRootWindow(WindowType_SSL, false, utils::GetDataURI(ss.str(), "text/html"));
  }
  ...
  void ClientHandler::NotifyAutoResize(const CefSize& new_size) {
    if (!CURRENTLY_ON_MAIN_THREAD()) {
      // Execute this method on the main thread.
      MAIN_POST_CLOSURE(base::Bind(&ClientHandler::NotifyAutoResize, this, new_size));
      return;
    }
